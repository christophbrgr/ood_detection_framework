...
def sample_estimator_cifar10(model, num_classes, feature_list, train_loader):
    """
    compute sample mean and precision (inverse of covariance)
    return: sample_class_mean: list of class mean
             precision: list of precisions
...
def get_Mahalanobis_score(model, data, num_classes, sample_mean, precision, num_output, magnitude):
    ...
    for layer_index in range(num_output):
        ...
        pure_gau = -0.5 * \
            torch.mm(torch.mm(zero_f, Variable(
                precision[layer_index])), zero_f.t()).diag()
        ...
        if layer_index == 0:
            ...
            Mahalanobis_scores = np.concatenate((Mahalanobis_scores, noise_gaussian_score.reshape(
                (noise_gaussian_score.shape[0], -1))), axis=1)
