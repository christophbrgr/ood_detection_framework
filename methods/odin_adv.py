...
def eval(path_in, path_out, net, testloader, oodloader, use_cuda=True, noiseMagnitude = 0.0014, temper = 1000, save_dir=None):
    f1 = open(path_in, 'w')
    f2 = open(path_out, 'w')
    ...
    if save_dir:
        with open(os.path.join(save_dir, 'odin_adv_targets.txt'), 'w') as f:
            for item in labels_list:
                f.write('{}\n'.format(item.cpu().numpy()[0]))
        with open(os.path.join(save_dir, 'odin_adv_pred.txt'), 'w') as f:
            for item in predicted_list:
                f.write('{}\n'.format(item))
        with open(os.path.join(save_dir, 'odin_adv_correct.txt'), 'w') as f:
            for item in correct_list:
                f.write('{}\n'.format(item))
        with open(os.path.join(save_dir, 'odin_adv_confidence.txt'), 'w') as f:
...
def eval_cifar10(path_in, path_out, net, testloader, oodloader, use_cuda=True, noiseMagnitude = 0.0014, temper = 1000, save_dir=None):
    ...
    acc_list = (sum(correct_list)/len(correct_list))
    if save_dir:
        with open(os.path.join(save_dir, 'odin_adv_targets_cifar10.txt'), 'w') as f:
            for item in labels_list:
                f.write('{}\n'.format(item))
        with open(os.path.join(save_dir, 'odin_adv_pred_cifar10.txt'), 'w') as f:
            for item in predicted_list:
                f.write('{}\n'.format(item))
        with open(os.path.join(save_dir, 'odin_adv_correct_cifar10.txt'), 'w') as f:
            for item in correct_list:
                f.write('{}\n'.format(item))
        with open(os.path.join(save_dir, 'odin_adv_confidence_cifar10.txt'), 'w') as f:
            for item in confidence_list:
                f.write('{}\n'.format(item))
    # calculate AUROC for classifcation accuracy
    fpr, tpr, _ = skm.roc_curve(y_true = correct_list, y_score = confidence_list, pos_label = 1) #positive class is 1; negative class is 0
